...
class SudokuForm (Form):
	...
	for i in range(9):
		for j in range(9):
			form.append(TextField(str(i*9+j)))
...
def sudokuSolver():
	form = SudokuForm(csrf_enabled=False)
	obj = {
		"url": "/solveSudoku",
		"post": "post",
		"board": form,
...
def solveSudoku():
	...
	for i in range(9):
		temp = []
		for j in range(9):
			if not request.form.get(str(i*9+j), None) == None:
				temp.append(str(request.form.get(str(i*9+j), None)))
			else:
				temp.append("0")
		board.append(temp)
	info = request.form
...
#		output.append(temp)
	...
	if info == None:
		...
		return render_template('sudokuSolver.jade')
